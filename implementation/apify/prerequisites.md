...
| Authentication | CLI authenticated to the intended Apify account and an API token available for API testing | Step 2 must deploy through the authenticated development workflow and separately verify API invocation | Action required | CLI authentication has been completed successfully. An API token for explicit external API testing has not yet been confirmed. | Create or identify an API token for Step 2 API invocation and store it without committing the secret to the repository. |
...
| Platform usage visibility | Ability to inspect individual Actor runs, logs and platform usage/cost information | Step 2 must verify runtime behaviour and observe resource consumption rather than merely obtain a successful result | Action required | Billing/limits visibility is confirmed, but run-specific logs and usage visibility have not yet been verified because no platform-baseline Actor has been run. | Confirm run status, logs and run-specific usage visibility during the first controlled baseline run. |
...
The account, CLI and cost-control prerequisites are now in place, and CLI authentication has been completed. The remaining setup items are to confirm an API token for explicit API testing and select the baseline runtime.

Run-specific logs and usage visibility will be confirmed during the first controlled Actor run because no run exists yet from which to verify that evidence.

No channel-level external dependency has been identified that would prevent the platform spike once the remaining setup items are completed.

## 5. Step 1 Completion

**Ready for Step 2:** No

**Open blockers:** API token for explicit API testing not yet confirmed; baseline runtime not yet selected; run-specific log/usage visibility still to be verified on the first controlled run.

The prerequisite set is identified and mostly satisfied. Step 1 remains open until the API token and baseline runtime are confirmed. Run-specific observability will then be verified immediately as part of the first controlled platform-baseline run.
